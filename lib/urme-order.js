...
function normalizeParticipantExtra(input = {}) {
  return {
    ...
    lastName: asString(input.lastName),
    workshop: asString(input.workshop),
    attendance: asString(input.attendance) || "absent"
  };
}
...
export function validateUrmeOrderInput(input = {}) {
  ...
  const attendance = asString(input.attendance) || "absent";
...
export function buildCreateOrderPayload(normalized) {
  const { mainParticipant, extraParticipants, totalQty, status, attendance } =
    normalized;
  ...
  return {
    ...
    meta_data: [
      ...buildMetaData({ mainParticipant, extraParticipants }),
      { key: "_urme_attendance", value: attendance },
    ],
  };
}
...
export function extractEditableOrder(order) {
  ...
  return {
    ...
    status: order.status,
    attendance: asString(getMeta("_urme_attendance")) || "absent",
    ...
    extraParticipants: participantsArr.map((p) => ({
      ...
      attendance: asString(p?.attendance) || "absent"
    })),
  };
}
